...
# Header and columns
    ...
    with col1:
        ...
        # Prepare empty grid or previous session data
        ...
        gb.configure_column("Item", cellEditor="agSelectCellEditor", cellEditorParams={"values": sorted(med_df["name"].dropna().unique().tolist())})
        
        # Optional: show row totals dynamically
        js_calculate = JsCode("""
        function(params) {
            let qty = params.data.Qty || 0;
            let rate = params.data.Rate || 0;
            let disc = params.data["Discount %"] || 0;
            let gst = params.data["GST %"] || 0;
            let amt = qty * rate * (1 - disc/100) * (1 + gst/100);
            return amt.toFixed(2);
        };
        """)
        gb.configure_column("Amount", valueGetter=js_calculate)
            ...
            if not party:
                ...
                for _, it in updated_df.iterrows():
                    item_dict = {
                        ...
                        "party": party,
                        "item": it["Item"],
                        "batch": it["Batch"] if it["Batch"] else "",
                        "qty": it["Qty"],
                        "rate": it["Rate"],
                        "mrp": it["MRP"],
